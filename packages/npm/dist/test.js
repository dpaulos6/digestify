...
function main() {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    console.log(
      `Password 16 chars: ${(0, keygen_1.generatePassword)({ length: 16, includeLowercase: true, includeUppercase: true, includeNumbers: true, includeSymbols: true })}`
    )
    ...
    console.log(
      `Hashed '${hashTxt}' using 'bcrypt': ${yield (0, hashing_1.hash)(hashTxt.trim(), 'bcrypt')}`
    )
    console.log(
      `Hashed '${hashTxt}' using 'sha512': ${yield (0, hashing_1.hash)(hashTxt.trim(), 'sha-512')}`
    )
    console.log(
      `Hashed '${hashTxt}' using 'md4': ${yield (0, hashing_1.hash)(hashTxt.trim(), 'md4')}`
    )
    console.log(
      `Hashed '${hashTxt}' using 'xxHash32': ${yield (0, hashing_1.hash)(hashTxt.trim(), 'xxHash32')}`
    )
    console.log(
      `Hashed '${hashTxt}' using 'ripemd-160': ${yield (0, hashing_1.hash)(hashTxt.trim(), 'ripemd-160')}`
    )
  })
}
